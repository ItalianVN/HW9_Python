...
def SaveStudents(student):
    with open('C:\python\sem9\students.csv', 'a') as file:
...
def GetLastStudentId():
    global student_id_counter
    with open('C:\python\sem9\last_student_id.txt', 'r',encoding='utf-8') as file:
        student_id_counter = int(file.read())


def SaveLastStudentId():
    global student_id_counter
    with open('C:\python\sem9\last_student_id.txt', 'w',encoding='utf-8') as file:
        file.write(str(student_id_counter))


def SaveClasses():
    with open('C:\python\sem9\classes.txt', 'w') as file:
